...
void DLL_Dispose( DLList *list ) {
	while (list->firstElement) {
		if (list->firstElement && list->lastElement && list->currentLength > 0) {
			DLLElementPtr tmp = list->firstElement->nextElement; // Prvek nebo NULL
...
void DLL_DeleteLast( DLList *list ) {
	if (list->firstElement && list->lastElement && list->currentLength > 0) {
		DLLElementPtr tmp = list->lastElement->previousElement; // Prvek nebo NULL
...
void DLL_DeleteAfter( DLList *list ) {
	if (list->activeElement != NULL && (list->activeElement != list->lastElement)) {
		...
		DLLElementPtr temp = list->activeElement->nextElement->nextElement; // Prvek nebo NULL
...
void DLL_DeleteBefore( DLList *list ) {
	if (list->activeElement != NULL && (list->activeElement != list->firstElement)) {
		...
		DLLElementPtr temp = list->activeElement->previousElement->previousElement; // Prvek nebo NULL
...
void DLL_InsertAfter( DLList *list, long data ) {
	if (list->activeElement != NULL) {
		...
		DLLElementPtr temp = list->activeElement->nextElement; // Prvek nebo NULL

		if (newElement) {
			newElement->data = data;
			newElement->previousElement = list->activeElement;
			newElement->nextElement = temp;
			++list->currentLength;
		} else {
			DLL_Error();
			return;
		...
		if (temp) {
			temp->previousElement = newElement;
		}
...
void DLL_InsertBefore( DLList *list, long data ) {
	if (list->activeElement != NULL) {
		...
		DLLElementPtr temp = list->activeElement->previousElement; // Prvek nebo NULL

		if (newElement) {
			newElement->data = data;
			newElement->nextElement = list->activeElement;
			newElement->previousElement = temp;
			++list->currentLength;
		} else {
			DLL_Error();
			return;
