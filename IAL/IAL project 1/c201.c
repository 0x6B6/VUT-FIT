...
void List_Dispose( List *list ) {
	while (list->firstElement) {
		if (list->firstElement && list->currentLength > 0) {
			ListElementPtr tmp =  list->firstElement->nextElement; // Prvek nebo NULL
...
void List_DeleteAfter( List *list ) {
	if (List_IsActive(list) && list->activeElement->nextElement) {
		ListElementPtr tmp = list->activeElement->nextElement->nextElement; // Prvek nebo NULL
...
void List_InsertAfter( List *list, int data ) {
	if (List_IsActive(list)) {
		...
		ListElementPtr temp = list->activeElement->nextElement; // Prvek nebo NULL
